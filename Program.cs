...
string[] actions = new[] { UP, DOWN, LEFT, RIGHT };

TorchSharp.torch.Tensor rewards;
TorchSharp.torch.Tensor qValues;
...
void setupRewards(int[,] maze, int wallValue, int floorValue, int goalValue)
{
    ...
    var rewardsList = new List<float>();
    ...
    for (int i = 0; i < mazeRows; i++)
    {
        for (int j = 0; j < mazeColumns; j++)
        {
            ...
            {
                case 0:
                    rewardsList.Add(wallValue);
                    break;
                case 1:
                    rewardsList.Add(floorValue);
                    break;
                case 2:
                    rewardsList.Add(goalValue);
                    break;
                default:
                    rewardsList.Add(floorValue);
                    break;
    ...
    rewards = tensor(rewardsList.ToArray(), new long[] { mazeRows, mazeColumns }, torch.float32);
}
...
void setupQValues(int[,] maze)
{
    int mazeRows = maze.GetLength(0);
    int mazeColumns = maze.GetLength(1);
    qValues = zeros(new long[] { mazeRows, mazeColumns, 4 }, torch.float32);
}

bool hasHitWallOrEndOfMaze(int currentRow, int currentColumn, int floorValue)
{
    var rewardValue = rewards[currentRow, currentColumn].item<float>();
    return rewardValue != floorValue;
}

int determineNextAction(int currentRow, int currentColumn, float epsilon)
{
    Random random = new Random();
    double randomBetween0And1 = random.NextDouble();
    
    if (randomBetween0And1 < epsilon)
    {
        var qValuesSlice = qValues[currentRow, currentColumn];
        long maxIndex = argmax(qValuesSlice).item<long>();
        return (int)maxIndex;
    }
    else
    {
...
(int, int) moveOneSpace(int[,] maze, int currentRow, int currentColumn, int currentAction)
{
    int mazeRows = maze.GetLength(0);
    int mazeColumns = maze.GetLength(1);
...
void trainTheModel(int[,] maze, int floorValue, float epsilon, float discountFactor, float learningRate, int episodes)
{
    for (int episode = 0; episode < episodes; episode++)
    {
        ...
        {
            int currentAction = determineNextAction(currentRow, currentColumn, epsilon);
            ...
            float reward = rewards[currentRow, currentColumn].item<float>();
            float previousQValue = qValues[previousRow, previousColumn, currentAction].item<float>();
            
            var maxQNextState = max(qValues[currentRow, currentColumn]).item<float>();
            float temporalDifference = reward + (discountFactor * maxQNextState);
...
List<int[]> navigation(int[,] maze, int startRow, int startColumn, int floorValue, int wallValue)
{
    ...
    while (!hasHitWallOrEndOfMaze(currentRow, currentColumn, floorValue))
    {
        int nextAction = determineNextAction(currentRow, currentColumn, 1.0f);
        (int, int) nextMove = moveOneSpace(maze, currentRow, currentColumn, nextAction);
        currentRow = nextMove.Item1;
        currentColumn = nextMove.Item2;

        if (rewards[currentRow, currentColumn].item<float>() != wallValue)
        {
...
setupRewards(maze1, WALL_REWARD, FLOOR_REWARD_VALUE, GOAL_REWARD_VALUE);
setupQValues(maze1);
trainTheModel(maze1, FLOOR_REWARD_VALUE, EPSILON, DISCOUNT_FACTOR, LEARNING_RATE, EPISODES);
navigation(maze1, Start_Row, Start_Column, FLOOR_REWARD_VALUE, WALL_REWARD);
